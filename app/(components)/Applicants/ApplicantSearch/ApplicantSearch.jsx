...
const SearchInput = ({searchIconToggle}) => {
  ...
  return (
    <div className="relative xl:w-[450px] max-w-lg mx-auto">
      ...
      />
      <div className={`absolute flex items-center  cursor-pointer ${locale === "ar" ? " inset-y-0 left-0 pl-3" : " inset-y-0 right-0 pr-3"} ${searchIconToggle}`}>
        <Image src={filterIcon} alt="Filter Icon" width={24} height={24} onClick={showModal} />
      </div>
      <div className={` absolute flex items-center ${locale === "ar" ? "inset-y-0 right-0 " : "inset-y-0 left-0 "} ${searchIconToggle}`}>
        <Image src={searchIcon} alt="Search Icon" width={24} height={24}  />
      </div>
      <ApplicantModal visible={isModalVisible} onClose={closeModal} />
    </div>
  );
};
